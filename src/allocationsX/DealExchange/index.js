  ...
  return (
    <Paper className="OrderForm">
      <div className="choose-direction">
        <span
          className="direction direction-buy"
          data-selected={order.direction === 'buy'}
          onClick={() => setOrder({ direction: 'buy' })}
        >
          Buy
        </span>
        <span
          className="direction direction-sell"
          data-selected={order.direction === 'sell'}
          onClick={() => setOrder({ direction: 'sell' })}
        >
          Sell
        </span>
      </div>
        ...
        InputProps={{
          endAdornment: (
            <InputAdornment
              ...
              position="end"
              onClick={() => setOrder({ amount: exchangeDeal.shares })}
            >
              max
            </InputAdornment>
          ),
      ...
      <div className="order-details">
        ...
        <Button variant="contained" size="small" color="primary" onClick={submit}>
          SUBMIT
        </Button>
      </div>
    </Paper>
...
function Order({ investor, _id, user_id, refetch, side, price, amount }) {
  ...
  const rowStyle = investor._id === user_id ? { backgroundColor: '#ffffe0' } : {};

  const actionButton =
    investor._id === user_id ? (
      <Button variant="contained" className="cancel-btn" onClick={cancel}>
        CANCEL
      </Button>
    ) : (
      <Button

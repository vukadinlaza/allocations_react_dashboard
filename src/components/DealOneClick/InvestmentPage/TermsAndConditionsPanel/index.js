...
function TermsAndConditionsPanel({
  deal: { _id, documents, signDeadline, status },
  checkedTAT,
  setCheckedTAT,
  confirmInvestment,
  isEdit,
  ...
  useEffect(() => {
    (async () => {
      const res = await fetch(`${process.env.REACT_APP_BUILD_FRONTEND_URL}/api/deals/${_id}`);
  ...
  return (
    <section className="TermsAndConditions">
      <div className="terms-container">
        ...
        {serviceDeal?.metadata?.terms ? (
          <Box p={3} dangerouslySetInnerHTML={{ __html: serviceDeal.metadata.terms }} />
        ) : (
          <Box p={3}>
            <Typography>
            ...
            <Typography>
              {titleCase(weOrI)} have read and agree to the Portfolio Company’s investment
              agreement, which is attached to the SPV Documents and will be executed between the
              Portfolio Company and the SPV.
            </Typography>
            <Typography>

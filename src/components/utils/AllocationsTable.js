	...
	return (
		<div className={classes.root} style={{ width: "100%" }}>
			...
			>
				...
				<Table className={classes.table} size={tableSize ? tableSize : "medium"} aria-label="simple table">
					<TableHead>
						<TableRow>
							{includeCheckBox ?
								<TableCell align="center" padding="checkbox">
									<Checkbox
										...
										checkedIcon={<FiberManualRecordIcon fontSize="small" className={classes.selectedCheckbox} />}
									/>
								</TableCell> :
								null
							}
					...
					<TableBody>
						{(data && data.length) ? dataToShow.map((row, idx) => {
							...
							return (
								...
								>
									...
									{(headers && headers.length) ? headers.map((header, i) =>
										(Object.keys(row).includes(header.value) || header.keyNotInData ?
											...
											<TableCell key={`${header.value}-${i}`}></TableCell>
										)) : null
									}
								</TableRow>
							)

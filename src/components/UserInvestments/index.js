...
const TR = ({ investment, showDocs, setShowDocs }) => {
  ...
  return (
    <TableRow key={investment._id} className="investment-row">
      <TableCell style={{ maxWidth: '200px', minWith: '200px', width: '200px' }} scope="row">
        {investment.deal.company_name}
      </TableCell>
      <Hidden xsDown>
        <TableCell style={{ maxWidth: '300px', minWith: '300px', width: '300px' }}>
      ...
      <TableCell align="right" style={{ display: 'flex', justifyContent: 'space-between' }}>
        {investment?.documents?.length >= 1 && (
          <Button
            ...
            size="small"
            color="primary"
            onClick={() => setShowDocs(showDocs ? null : investment)}
          >
            Documents
          </Button>
        )}
...
export default function UserInvestments() {
  ...
  if (showDocs) {
    type =
      showDocs.status === 'complete'
        ? completeInvestments
        : showDocs.status === 'invited'
        ? invitedInvestments
  ...
  return (
    <>
      ...
      <>
        <Tab
          investments={showDocs ? type : activeInvestments.investments || invitedInvestments}
          showDocs={showDocs}
          setShowDocs={setShowDocs}
          type={activeInvestments.status}
        />
      </>
    </>
  );
